...
column id1 format 999999990
column id2 format 999999990
column locking heading 'Lock Held/Lock Requested' format a40
column lmode_ heading 'Lock Held' format a20
column request_ heading 'Lock Requested' format a20
column serial# format 99999
column username  format a23  heading "Username"
column terminal heading Term format a6
column tab format a50 heading "Table Name"
...
nvl(S.USERNAME,'Internal') username,
        ...
        /*decode(command,
0,'None',*/
case when l.type = 'UL' then (select dla.name from sys.dbms_lock_allocated dla where dla.lockid = id1)
     else decode(l.id2,0,initcap(replace(replace(replace(T1.owner,'OWN_'),'OWNER_'),'_TAB'))||'.'||substr(T1.object_NAME,1,30),'None')/*)*/
end tab,
decode(command,
0,'BACKGROUND',
1,'Create Table',
...
command||' - ???') COMMAND,
        decode(L.LMODE,1,'No Lock',
                ...
                6,'Exclusive','NONE') lmode_,
        decode(L.REQUEST,1,'No Lock',
                2,'Row Share',
                3,'Row Exclusive',
                4,'Share',
                5,'Share Row Exclusive',
                6,'Exclusive','NONE') request_,
l.id1||'-'||l.id2 Laddr,
l.type||' - '||
decode(l.type,
