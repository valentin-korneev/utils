...
col time for a18 hea "Time(/Total)"
...
select
  ...
  , ltrim((last_update_time - start_time) day(9) to second(0), '+0 :')
    || case when sofar <> totalwork then
       '/'|| ltrim(cast((last_update_time - start_time) day to second*(totalwork/nullif(sofar,0)) as interval day(9) to second(0)), '+0 :')
       end as time
  , username
  , sql_id
